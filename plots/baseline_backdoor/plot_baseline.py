...
for i in range(5):
	df = pd.read_csv("increasing_back_mean_" + str(i) + ".csv", header=None)
	data1[:, i] = df.values[:, 2]

for i in range(5):
	df = pd.read_csv("increasing_back_krum_" + str(i) + ".csv", header=None)
	data2[:, i] = df.values[:, 2]

for i in range(5):
	df = pd.read_csv("increasing_back_median_" + str(i) + ".csv", header=None)
	data3[:, i] = df.values[:, 2]

for i in range(5):
	df = pd.read_csv("increasing_back_trimmean10_" + str(i) + ".csv", header=None)
	data4[:, i] = df.values[:, 2]

for i in range(5):
	df = pd.read_csv("increasing_back_trimmean20_" + str(i) + ".csv", header=None)
	data5[:, i] = df.values[:, 2]

for i in range(5):
	df = pd.read_csv("increasing_back_fg_" + str(i) + ".csv", header=None)
	data6[:, i] = df.values[:, 2]
...
l4 = mlines.Line2D(np.arange(10), np.mean(data4, axis=1), label="Trimmed Mean (B = 0.1)", marker='s', color='red', markersize=10)
